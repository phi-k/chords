...
class ChordTransposer {
  ...
  static String _getTransposedNote(
      String note, int semitones, bool preferFlats) {
  ...
  static String transposeChord(String chord, int semitones,
    ...
    if (!ChordDetector.isValidChord(ChordDetector.cleanToken(chord))) {
      return chord;
    }

    bool startsWithParenthesis = chord.startsWith('(');
    String chordWithoutParentheses =
        startsWithParenthesis ? chord.substring(1, chord.length - 1) : chord;

    final rootMatch =
        RegExp(r'^([A-G][#b]?)').firstMatch(chordWithoutParentheses);
    ...
    String transposedRoot =
        _getTransposedNote(rootNote, semitones, preferFlats);
    ...
    if (remainder.contains('/')) {
      ...
      if (bassMatch != null) {
        ...
        String transposedBass =
            _getTransposedNote(bassRoot, semitones, preferFlats);
  ...
  static String transposeLyricsWithChords(
      String lyricsWithChords, int semitones) {
      ...
      if (ChordDetector.isChordLine(normalizedLine)) {
        ...
        for (var match in matches) {
          ...
          if (token.contains('-')) {
            ...
            final transposedSubs = subTokens.map((sub) {
              return transposeChord(sub, semitones, preferFlats: preferFlats);
            }).join('-');
            newLine.write(transposedSubs);
          } else {
            newLine.write(
                transposeChord(token, semitones, preferFlats: preferFlats));
          }
